const Shimmer = () => {
    return(
        <div className="m-4 p-4">
            <div className="shimmer-filter">
                <input type="text" className="shimmer-input p-2 border border-solid border-black" />
                
                <button className="shimmer-btn-filter border border-green-300 rounded-md px-5 py-2 bg-green-200 m-4 cursor-pointer"></button>
            </div>
            <div className="shimmer-container grid grid-cols-6">
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img w-40 border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black"  />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
                <div className="shimmer-cards m-4 p-4 w-60 h-auto bg-gray-200 rounded-md">
                    <img className="cart-img border border-black" />
                    <h3 className="shimmer-h3"></h3>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                    <h4 className="shimmer-h4"></h4>
                </div>
            </div>
        </div>
    )
}

export default Shimmer;